...
def g_r(s,ter,_ax,c,k,_plot=True):
    ...
    (rho*T)+np.trapz(arg_p*data[:,1],data[:,0])*2.*np.pi*(rho**2)/3.])
    ...
    if (rho<0.31 or rho>0.05) and _plot:
        _ax.plot(data[:,0],5-k+data[:,1],color=c,linewidth=2.5)
        _ax.text(data[-1,0]*0.8,(5-k+data[-1,1])+0.4,f'$\\rho:${rho:2.2}')
        _ax.axhline(5-k+1,color='green',linestyle='--',linewidth=0.5)
